...
def parse_tsw1400(
    ...
    assert adc_data.shape[0] == num_sample, (
        "Actual number of samples (%d) doesn't equal to expected (%d)"
        % (adc_data.shape[0], num_sample)
    )
    ...
    if iq:
        adc_deinterleaved = [
            adc_data[:, :, i::channel_count] for i in range(channel_count)
        ]  # i = 0, 1, channel_count = 2
        adc_data = adc_deinterleaved[0] + 1j * adc_deinterleaved[1]

    # adc_data *= normFactor
    assert adc_data.shape == (num_chirps, num_ants, num_adc_samples), (
        "ADC data is not parsed to desired shape. Currently it is {}".format(
            adc_data.shape
        )
    )

    adc_data = adc_data.reshape(
        num_frames, num_chirps_per_frame, num_ants, num_adc_samples
    )
