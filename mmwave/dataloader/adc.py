...
class DCA1000:
    ...
    def AWR2243_read_config(config_file_name):
        ...
        for line in config:
            ...
            if "startFreqConst" == List[0]:
                if curProfileId == 0:  # 只取第一个profile
                    ADC_PARAMS["startFreq"] = int(List[1].split(";")[0]) * (
                        3.6 / (1 << 26)
                    )
            ...
            if "freqSlopeConst" == List[0]:
                if curProfileId == 0:  # 只取第一个profile
                    ADC_PARAMS["freq_slope"] = int(List[1].split(";")[0]) * (
                        3.6e3 * 900 / (1 << 26)
                    )
        ...
        if ADC_PARAMS["tx"] > CFG_PARAMS["numTxChan"]:
            raise ValueError(
                ...
                % (CFG_PARAMS["numTxChan"], ADC_PARAMS["tx"])
            )
        ...
        for i in range(ADC_PARAMS["tx"] - 1):
            ...
            if tmp_chirpNum != chirpEndIdx_buf[i + 1] - chirpStartIdx_buf[i + 1] + 1:
                raise ValueError(
                    "AWR2243_read_config does not support different chirp number in different tx ant yet."
                )
    ...
    def read_config(config_file_name):
        ...
        for line in config:
            ...
            if "frameCfg" in line:
                ...
                numOfChirps_buf.append(
                    CFG_PARAMS["chirpEndIdx"] - CFG_PARAMS["chirpStartIdx"] + 1
                )
        ...
        if lvdsCfg_headerEn_buf[0] == 0:
            ...
            if (
                lvdsCfg_dataFmt_buf[0] == 1 or lvdsCfg_dataFmt_buf[0] == 4
            ):  # 1:ADC 4:CP+ADC+CQ
        ...
        for subframeIdx in range(1, CFG_PARAMS["numSubframes"]):
            if (
                lvdsCfg_dataFmt_buf[subframeIdx] == 1
                or lvdsCfg_dataFmt_buf[subframeIdx] == 4
            ):  # 1:ADC 4:CP+ADC+CQ
    ...
    def configure(self, FPGAjsonFilePath, IWRcfgFilePath):
        ...
        LVDSDataSizePerChirp_l, maxSendBytesPerChirp_l, ADC_PARAMS_l, CFG_PARAMS_l = (
            self.read_config(IWRcfgFilePath)
        )
    ...
    def read(self, timeout=1):
        ...
        while True:
            ...
            try:
                ret_frame[
                    curr_idx * UINT16_IN_PACKET : (curr_idx + 1) * UINT16_IN_PACKET
                ] = packet_data
            except Exception as e:
                print(f"Exception inattendue : {e.__class__.__name__} - {e}")
                raise  # pour relancer l'exception après affichage (utile en debug)
    ...
    def postProcPacket(self, recvQueue, maxPacketNum):
        ...
        while recvQueue:
            ...
            receivedData[UINT16_IN_PACKET * idx : UINT16_IN_PACKET * (idx + 1)] = (
                packet_data
            )
    ...
    def fastRead_from0(self, numframes=1):
        ...
        receivedData, firstPacketNum, receivedPacketNum = self.postProcPacket(
            recvQueue, packetNum
        )
        databuf = receivedData[0 : numframes * UINT16_IN_FRAME]
        print(
            "received packet num:%d,expected packet num:%d"
            % (len(receivedPacketNum), packetNum)
        )
    ...
    def fastRead_in_Cpp(self, numframes=1, timeOut=2, sortInC=True):
        ...
        if sortInC:  # sort packet using C code (True) or python (False)
            recvData = np.ndarray(
                shape=-1, dtype=np.int16, buffer=recvData
            )  # convert to int16
            ...
            print(
                "first Packet Num:%d,last Packet Num:%d"
                % (firstPacketNum, lastPacketNum)
            )
            print(
                "received packet num:%d,expected packet num:%d,loss:%.2f%%"
                % (
                    receivedPacketNum,
                    expectedPacketNum,
                    (expectedPacketNum - receivedPacketNum) / expectedPacketNum * 100,
                )
            )
            return recvData
        else:
            ...
            print(
                "received packet num:%d,expected packet num:%d,loss:%.2f%%"
                % (
                    len(receivedPacketNum),
                    minPacketNum,
                    (minPacketNum - len(receivedPacketNum)) / minPacketNum * 100,
                )
            )
    ...
    def fastRead_in_Cpp_thread_start(self, frameNumInBuf=2):
        ...
        print(
            "allocated udp buffer length:",
            ret,
            "size(MB):",
            ret * BYTES_OF_PACKET / 1024 / 1024,
        )
    ...
    def fastRead_in_Cpp_thread_get(
        self, numframes=1, timeOut=2, verbose=False, sortInC=True
    ):
        if self.frameNumInBuf < numframes:
            raise ValueError(
                f"frameNumInBuf={self.frameNumInBuf} must bigger than numframes={numframes}"
            )
        minPacketNum = math.ceil(PACKETS_IN_FRAME * numframes)
        if verbose:
            print("min Packet Num:", minPacketNum)
        recvData = fpga_udp.udp_read_thread_get_frames(
            numframes, BYTES_IN_FRAME, timeOut, sortInC
        )
        if sortInC:  # sort packet using C code (True) or python (False)
            recvData = np.ndarray(
                shape=-1, dtype=np.int16, buffer=recvData
            ...
            if verbose:
                ...
                print(
                    ...
                    % (
                        receivedPacketNum,
                        expectedPacketNum,
                        (expectedPacketNum - receivedPacketNum)
                        / expectedPacketNum
                        * 100,
                    )
                )
    ...
    def sys_alive_check(self):
        ...
        if res:
            ...
            if (
                ...
                or res[1]
                != struct.unpack(
                    "<H", codecs.decode(str(CMD.SYSTEM_CONNECT_CMD_CODE), "hex")
                )[0]
                or res[3] != FOOTER_Num
            ):
    ...
    def read_fpga_version(self):
        ...
        if res:
            ...
            if (
                ...
                or res[1]
                != struct.unpack(
                    "<H", codecs.decode(str(CMD.READ_FPGA_VERSION_CMD_CODE), "hex")
                )[0]
                or res[3] != FOOTER_Num
            ):
                print("receive upd packet error")
    ...
    def config_fpga(self, filepath):
        ...
        with open(filepath, "r", encoding="utf8") as fp:
            ...
            payload = (
                ConfigLogMode[json_data["DCA1000Config"]["dataLoggingMode"]]
        ...
        packLen = str(
            codecs.encode(struct.pack("<H", len(codecs.decode(payload, "hex"))), "hex"),
            encoding="utf-8",
        )
        ...
        if res:
            ...
            if (
                ...
                or res[1]
                != struct.unpack(
                    "<H", codecs.decode(str(CMD.CONFIG_FPGA_GEN_CMD_CODE), "hex")
                )[0]
                or res[3] != FOOTER_Num
            ):
                print("receive upd packet error")
    ...
    def config_record(self, filepath):
        ...
        bytesPerPacket = str(
            codecs.encode(struct.pack("<H", MAX_BYTES_PER_PACKET), "hex"),
            encoding="utf-8",
        )
        reserved = "0000"
        ...
        if res:
            ...
            if (
                ...
                or res[1]
                != struct.unpack(
                    "<H", codecs.decode(str(CMD.CONFIG_PACKET_DATA_CMD_CODE), "hex")
                )[0]
                or res[3] != FOOTER_Num
            ):
                print("receive upd packet error")
    ...
    def reset_fpga(self):
        ...
        if res:
            ...
            if (
                ...
                or res[1]
                != struct.unpack(
                    "<H", codecs.decode(str(CMD.RESET_FPGA_CMD_CODE), "hex")
                )[0]
                or res[3] != FOOTER_Num
            ):
                print("receive upd packet error")
    ...
    def reset_radar(self):
        ...
        if res:
            ...
            if (
                ...
                or res[1]
                != struct.unpack(
                    "<H", codecs.decode(str(CMD.RESET_AR_DEV_CMD_CODE), "hex")
                )[0]
                or res[3] != FOOTER_Num
            ):
                print("receive upd packet error")
    ...
    def stream_start(self):
        ...
        if res:
            ...
            if (
                ...
                or res[1]
                != struct.unpack(
                    "<H", codecs.decode(str(CMD.RECORD_START_CMD_CODE), "hex")
                )[0]
                or res[3] != FOOTER_Num
            ):
                print("receive upd packet error")
    ...
    def stream_stop(self):
        ...
        if res:
            ...
            if (
                ...
                or res[1]
                != struct.unpack(
                    "<H", codecs.decode(str(CMD.RECORD_STOP_CMD_CODE), "hex")
                )[0]
                or res[3] != FOOTER_Num
            ):
                print("receive upd packet error")
