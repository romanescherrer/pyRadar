...
class TI:
    ...
    def __init__(
        ...
        connect=True,
        mode=0,
        config_file="./xwr18xx_profile.cfg",
    ):
    ...
    def _initialize(self, config_file="./xwr18xx_profile.cfg"):
        ...
        num_chirps_per_frame = (
            self.chirp_end_idx - self.chirp_start_idx + 1
        ) * self.num_loops
        ...
        self.config_params["rangeResolutionMeters"] = (
            3e8 * dig_out_sample_rate * 1e3
        ...
        self.config_params["dopplerResolutionMps"] = 3e8 / (
            ...
            * 1e-6
            * self.config_params["numDopplerBins"]
            * self.num_tx_ant
        )
        self.config_params["maxRange"] = (300 * 0.9 * dig_out_sample_rate) / (
    ...
    def create_read_process(self, numframes=0):
        if self.data_process_handle is None:
            ...
            BYTES_IN_FRAME = (
                self.num_loops * self.num_virtual_ant * self.num_adc_samples * 4
            )
            ...
            self.data_process_handle = fpga_udp.radar_start_read_thread(
                self.data_loc, self.data_baud, packetNum, BYTES_IN_PACKET
            )
    ...
    def parseData18xx(self, readBuffer):
        ...
        if self.byteBufferLength > 16:
            ...
            if startIdx:
                ...
                if (self.byteBufferLength >= totalPacketLen) and (
                    self.byteBufferLength != 0
                ):
                    magicOK = 1
            ...
            for tlvIdx in range(numTLVs):
                ...
                if tlv_type == MMWDEMO_UART_MSG_DETECTED_POINTS:
                    ...
                    for objectNum in range(numDetectedObj):
                        ...
                        x[objectNum] = self.byteBuffer[idX : idX + 4].view(
                            dtype=np.float32
                        )
                        idX += 4
                        y[objectNum] = self.byteBuffer[idX : idX + 4].view(
                            dtype=np.float32
                        )
                        idX += 4
                        z[objectNum] = self.byteBuffer[idX : idX + 4].view(
                            dtype=np.float32
                        )
                        idX += 4
                        velocity[objectNum] = self.byteBuffer[idX : idX + 4].view(
                            dtype=np.float32
                        )
                        idX += 4
                    ...
                    numBytes = (
                        2
                        * self.config_params["numRangeBins"]
                        * self.config_params["numDopplerBins"]
                    )
                    ...
                    rangeArray = (
                        np.array(range(self.config_params["numRangeBins"]))
                        * self.config_params["rangeIdxToMeters"]
                    )
                    dopplerArray = np.multiply(
                        np.arange(
                            -self.config_params["numDopplerBins"] / 2,
                            self.config_params["numDopplerBins"] / 2,
                        ),
                        self.config_params["dopplerResolutionMps"],
                    )
