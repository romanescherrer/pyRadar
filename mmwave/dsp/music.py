...
def aoa_music_1D(steering_vec, rx_chirps, num_sources):
    ...
    assert num_antennas == steering_vec.shape[1], (
        "Mismatch between number of receivers in "
    )
...
def aoa_spatial_smoothing(covariance_matrix, num_subarrays, forward_backward=False):
    ...
    assert num_subarrays >= 1 and num_subarrays <= num_receivers, (
        "num_subarrays is wrong"
    )

    # Forward pass
    result = covariance_matrix[
        : num_receivers - num_subarrays + 1, : num_receivers - num_subarrays + 1
    ].copy()
    for i in range(1, num_subarrays):
        result += covariance_matrix[
            i : i + num_receivers - num_subarrays + 1,
            i : i + num_receivers - num_subarrays + 1,
        ]
...
def aoa_esprit(steering_vec, rx_chirps, num_sources, displacement):
    ...
    E1 = v1[:, -num_sources:]
    E2 = v2[:, -num_sources:]
    C = np.concatenate((E1.T.conj(), E2.T.conj()), axis=0) @ np.concatenate(
        (E1, E2), axis=1
    )
    _, Ec = LA.eigh(C)
    Ec = Ec[::-1, :]
