...
def _generate_dft_sin_cos_table(dft_length):
    ...
    dft_sin_cos_table = np.cos(
        2 * np.pi * dft_sin_cos_table / dft_length
    ) + 1j * -np.sin(2 * np.pi * dft_sin_cos_table / dft_length)

    # 1/2, 1/3 and 2/3 bins
    bins = np.array([0.5, 1.0 / 3, 2.0 / 3])
    bins = np.cos(2 * np.pi * bins / dft_length) - 1j * np.sin(
        2 * np.pi * bins / dft_length
    )

    return dft_sin_cos_table, bins


def add_doppler_compensation(
    input_data, num_tx_antennas, doppler_indices=None, num_doppler_bins=None
):
    ...
    if num_tx_antennas == 1:
        ...
        raise ValueError(
            "the specified number of transimitters is currently not supported"
        )
    ...
    if doppler_indices is not None:
        if num_doppler_bins is not None:
            ...
            doppler_compensation_indices[
                doppler_compensation_indices[:] >= (num_doppler_bins / 2)
            ] -= num_doppler_bins
            doppler_compensation_indices = doppler_compensation_indices // 2
            doppler_compensation_indices[doppler_compensation_indices[:] < 0] += (
                num_doppler_bins
            )
    ...
    for azi_val in azimuth_values:
        Re = (
            exp_doppler_compensation.real * azi_val.imag
            - exp_doppler_compensation.imag * azi_val.real
        )
        Im = (
            exp_doppler_compensation.imag * azi_val.imag
            + exp_doppler_compensation.real * azi_val.real
        )
...
def near_field_correction(
    ...
    azimuth_input[num_rx_antennas : num_rx_antennas + num_rx_antennas] = azimuth_input[
        num_angle_bins:
    ]
    ...
    # AB, CB, DB, EB
    geometry_lines = np.array(
        [
            geometry_points["A"] - geometry_points["B"],
    ...
    range_in_millimeter = (
        detected_objects["rangeIdx"][idx] * range_resolution - range_resolution
    ) * 1000
    ...
    for i in range(num_angle_bins):
        theta = (
            i * theta_incrementation
            if i < num_angle_bins / 2
            else (i - num_angle_bins) * theta_incrementation
        )

        tx1 = np.sqrt(
            range_squared
            + geometry_lines_square[1]
            - range_in_millimeter * theta * geometry_lines[1] * 2
        )
        rx4 = np.sqrt(
            range_squared
            + geometry_lines_square[2]
            - range_in_millimeter * theta * geometry_lines[2] * 2
        )
        tx2 = np.sqrt(
            range_squared
            + geometry_lines_square[0]
            - range_in_millimeter * theta * geometry_lines[0] * 2
        )
        rx1 = np.sqrt(
            range_squared
            + geometry_lines_square[3]
            - range_in_millimeter * theta * geometry_lines[3] * 2
        )

        if range > 0:
            psi = (
                MMWDEMO_TWO_PI_OVER_LAMBDA * ((tx2 + rx1) - (rx4 + tx1)) - np.pi * theta
            )
            corrReal = np.cos(psi)
            corrImag = np.sin(-psi)

        out1CorrReal = (
            azimuth_output[num_angle_bins + i].real * corrReal
            + azimuth_output[num_angle_bins + i].imag * corrImag
        )
        out1CorrImag = (
            azimuth_output[num_angle_bins + i].imag * corrReal
            + azimuth_output[num_angle_bins + i].real * corrImag
        )

        azimuth_output[i] = (azimuth_output[i].real + out1CorrReal) + (
            azimuth_output[i].imag + out1CorrImag
        ) * 1j

    return


def dc_range_signature_removal(
    fft_out1_d,
    positive_bin_idx,
    negative_bin_idx,
    ...
    # Calibration
    if (
        calib_dc_range_sig_cfg.counter
        < calib_dc_range_sig_cfg.num_frames * num_tx_antennas
    ):
        # Accumulate
        calib_dc_range_sig_cfg.mean[0, : positive_bin_idx + 1] = np.sum(
            fft_out1_d[0::2, :, : positive_bin_idx + 1], axis=(0, 1)
        )
        calib_dc_range_sig_cfg.mean[0, positive_bin_idx + 1 :] = np.sum(
            fft_out1_d[0::2, :, negative_bin_idx:], axis=(0, 1)
        )

        calib_dc_range_sig_cfg.mean[1, : positive_bin_idx + 1] = np.sum(
            fft_out1_d[1::2, :, : positive_bin_idx + 1], axis=(0, 1)
        )
        calib_dc_range_sig_cfg.mean[1, positive_bin_idx + 1 :] = np.sum(
            fft_out1_d[1::2, :, negative_bin_idx:], axis=(0, 1)
        )

        calib_dc_range_sig_cfg.counter += 1

        if calib_dc_range_sig_cfg.counter == (
            calib_dc_range_sig_cfg.num_frames * num_tx_antennas
        ):
            # Divide
        ...
        # fft_out1_d -= mean
        fft_out1_d[0::2, :, : positive_bin_idx + 1] -= calib_dc_range_sig_cfg.mean[
            0, : positive_bin_idx + 1
        ]
        fft_out1_d[0::2, :, positive_bin_idx + 1 :] -= calib_dc_range_sig_cfg.mean[
            0, positive_bin_idx + 1 :
        ]
        fft_out1_d[1::2, :, : positive_bin_idx + 1] -= calib_dc_range_sig_cfg.mean[
            1, : positive_bin_idx + 1
        ]
        fft_out1_d[1::2, :, positive_bin_idx + 1 :] -= calib_dc_range_sig_cfg.mean[
            1, positive_bin_idx + 1 :
        ]
