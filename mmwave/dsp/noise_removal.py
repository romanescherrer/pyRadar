...
def peak_grouping_along_doppler(det_obj_2d, det_matrix, num_doppler_bins):
...
def range_based_pruning(
    det_obj_2d_raw, snr_thresh, peak_val_thresh, max_range, min_range, range_resolution
):
    ...
    """
    det_obj_2d = det_obj_2d_raw[
        (det_obj_2d_raw["rangeIdx"] >= min_range)
        & (det_obj_2d_raw["rangeIdx"] <= max_range)
    ]
    snr_idx1 = (det_obj_2d["SNR"] > snr_thresh[0, 1]) & (
        det_obj_2d["rangeIdx"] * range_resolution < snr_thresh[0, 0]
    )
    snr_idx2 = (
        (det_obj_2d["SNR"] > snr_thresh[1, 1])
...
def prune_to_peaks(
    det_obj2_d_raw, det_matrix, num_doppler_bins, reserve_neighbor=False
):
    ...
    if reserve_neighbor:
        ...
        is_neighbor_of_peak_next = (current_val > next_next_val) & (
            current_val > prev_val
        )
        is_neighbor_of_peak_prev = (current_val > prev_prev_val) & (
            current_val > next_val
        )

        pruned_idx = (
            (current_val > prev_val) & (current_val > next_val)
            | is_neighbor_of_peak_next
            | is_neighbor_of_peak_prev
        )
    else:
